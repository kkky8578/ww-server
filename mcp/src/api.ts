 ...
 * }
 *
 * 这里用 TypeScript 的泛型 T，
 * 表示 data 字段里的真实数据类型是不固定的。
 *
 * 比如：
 * - 获取用户信息时，T 是“用户信息对象”
 * - 获取消费记录时，T 是“消费记录列表”
...
export async function getCurrentUserInfo() {
  return get<Record<string, unknown>>('/user/info');
}
...
export async function getUserConsumptionRecords() {
  return get<Record<string, unknown>>('/user/consumption-records');
}
 ...
 */
export async function getResumeQuizHistory(page: number, limit: number) {
  return get<Record<string, unknown>>(
    `/interview/resume/quiz/history?page=${page}&limit=${limit}`,
  );
}
 ...
 */
export async function getResumeQuizResultDetail(resultId: string) {
  return get<Record<string, unknown>>(
